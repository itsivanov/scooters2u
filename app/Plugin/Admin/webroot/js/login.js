$(document).ready(function () {	  
    ...
    $('.logo').show().delay(1200).animate({   opacity: 1,top: '7%' }, 300,function(){
...
function loading(form) {
    $('body').append('<div id="overlay"></div><div id="preloader">Checking...</div>');
    $('#overlay').css('opacity', 0.1).fadeIn(function () {
        $('#preloader').fadeIn();
    });
    var msgLogin = $('#msgLogin');
    $.post("/admin/users/login", form.serialize(),
        function (data) {
            if (!data.error) {
                unloading();
                $("#login").animate({   opacity: 1,top: '49%' }, 200,function(){
                    ...
                    $("#login").animate({   opacity: 0,top: '60%' }, 500,function(){
                        $(this).fadeOut(200,function(){
                            $(".text_success").slideDown();
                            $("#successLogin").animate({opacity: 1,height: "200px"},500, function(){
                ...
                unloading();
                msgLogin.text(data.err_desc).fadeIn();
            }
...
function unloading() {
    $('#preloader').fadeOut('fast',function(){ $('#preloader').remove(); $('#overlay').fadeOut(); });
}